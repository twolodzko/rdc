#!/usr/bin/env bats
# shellcheck disable=SC2016

@test "Greeting" {
	run ./rdc '[hello, world!]p'
	[ "$output" = "hello, world!" ]
	[ "$status" -eq 0 ]
}

@test "Push and print 123456789" {
	run ./rdc '123456789 p'
	[ "$output" = "123456789" ]
	[ "$status" -eq 0 ]
}

@test "Push and print 123456789.0" {
	run ./rdc '123456789.0 p'
	[ "$output" = "123456789" ]
	[ "$status" -eq 0 ]
}

@test "Push and print 123.456789" {
	run ./rdc '123.456789 p'
	[ "$output" = "123.456789" ]
	[ "$status" -eq 0 ]
}

@test "Push and print 123456.789" {
	run ./rdc '123456.789 p'
	[ "$output" = "123456.789" ]
	[ "$status" -eq 0 ]
}

@test "Push and print 0.123456789" {
	run ./rdc '0.123456789 p'
	[ "$output" = "0.123456789" ]
	[ "$status" -eq 0 ]
}

@test "Push and print 0.000123456" {
	run ./rdc '0.000123456 p'
	[ "$output" = "0.000123456" ]
	[ "$status" -eq 0 ]
}

@test "Push and print -123456789" {
	run ./rdc '_123456789 p'
	[ "$output" = "-123456789" ]
	[ "$status" -eq 0 ]
}

@test "2 + 2 = 4" {
	run ./rdc '2 2 + p'
	[ "$output" = "4" ]
	[ "$status" -eq 0 ]
}

@test "3 * 2 = 6" {
	run ./rdc '3 2 * p'
	[ "$output" = "6" ]
	[ "$status" -eq 0 ]
}

@test "14 / 2 = 7" {
	run ./rdc '14 2 / p'
	[ "$output" = "7" ]
	[ "$status" -eq 0 ]
}

@test "-2^2 = 4" {
	run ./rdc '_2 2 ^ p'
	[ "$output" = "4" ]
	[ "$status" -eq 0 ]
}

@test "Square value using duplication" {
	run ./rdc '2d*p'
	[ "$output" = "4" ]
	[ "$status" -eq 0 ]
}

@test "Square root of 25" {
	run ./rdc '25vp'
	[ "$output" = "5" ]
	[ "$status" -eq 0 ]
}

@test "Print stack" {
	run ./rdc '1 2 3 f'
   [ "$output" = "1 2 3" ]
	[ "$status" -eq 0 ]
}

@test "Clean stack" {
	run ./rdc '1 2 3 cf'
   [ "$output" = "" ]
	[ "$status" -eq 0 ]
}


@test "Reverse stack" {
	run ./rdc '1 2 3 rf'
   [ "$output" = "1 3 2" ]
	[ "$status" -eq 0 ]
}

@test "Execute string" {
	run ./rdc '[40 2 +p]x'
   [ "$output" = "42" ]
	[ "$status" -eq 0 ]
}

@test "Use register" {
	run ./rdc '2 sa 4 la /p'
   [ "$output" = "2" ]
	[ "$status" -eq 0 ]
}

@test "Empty register is zero" {
	run ./rdc 'lxp'
   [ "$output" = "0" ]
	[ "$status" -eq 0 ]
}

@test "Calculate factorial" {
	run ./rdc '5 [d1-d1<F*]dsFxp'
   [ "$output" = "120" ]
	[ "$status" -eq 0 ]
}

@test "Conditional call on 2 > 1" {
	run ./rdc '[[ok\]p]sa 1 2>a'
   [ "$output" = "ok" ]
	[ "$status" -eq 0 ]
}

@test "Conditional call on 8/2 = 3+1" {
	run ./rdc '[[also ok\]p]sa 8 2 / 3 1 + =a'
   [ "$output" = "also ok" ]
	[ "$status" -eq 0 ]
}

@test "Error on printing empty stack" {
	run ./rdc 'p'
   [ "$output" = "Error: empty stack" ]
	[ "$status" -ne 0 ]
}

@test "Error on adding NaNs" {
	run ./rdc '[] [] +'
   [ "$output" = "Error: not a number" ]
	[ "$status" -ne 0 ]
}
